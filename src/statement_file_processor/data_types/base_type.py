...
class BaseType(ABC):
    ...
    def ingest(self, json_object: Optional[Dict[str, Any]]) -> BaseType:
        '''
        For eg:
        Class Defenition
            ...
            if attribute_value is not None:
                ...
                if field in _transform_map:
                    attribute_value = _transform_map[field](
                        attribute_value)
                if isinstance(attribute_value, BaseType):
                    attribute_value = ~attribute_value
                if attribute_value is not None:
                    kwargs[self.get_key_appender()+field] = attribute_value
        return kwargs

    def _function_set(self, field: str) -> Callable[[str], BaseType]:
        #return lambda x: self._set_value(field, x)
        return partial(self._set_value,field = field)

    def _function_get(self, field: str) -> Callable[[], Any]:
        # return lambda: self._get_value(field)
        return partial(self._get_value, field)

    def _set_value(self, value: Any, field: str):
